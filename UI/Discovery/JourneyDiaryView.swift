...
/// **A journey's diary** — the Journey Discovery beta's detail screen, and one
/// action at the foot of it: *Make this a Film*.
///
...
/// place it reached, the photographs taken there. Photographs sit inside the
/// days they belong to, at the size of evidence, never as a cover.
///
/// ⚠️ **This does not replace `TripDetailView`** (Chiu, 2026-09-18). S3 is the
/// shipping trip screen and is untouched; this is the beta's own detail, so the
/// new language can be refined without the original path moving under it. Both
/// read the same `TripDetailModel`, so neither can show a different journey.
struct JourneyDiaryView: View {
