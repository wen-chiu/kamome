...
class Track:
    ...
    def walk_loop(self, center, minutes, speed_kmh, t, radius_m=250, sample_s=10):
        ...
        while cur <= end:
            ...
            r = radius_m * min(1.0, min(f, 1 - f) * 6)
...
def perth_margaret_river():
    ...
    clock = t.dwell(bunbury, 1, clock)                         # brief return to car (< dwell window)
    ...
    clock = t.dwell(busselton, 1, clock)
    ...
    t.write("perth_margaret_river_day1.gpx", [
        ...
        "2 walk loops (4-4.5 km/h, 10 s sampling): Bunbury 25 min, Busselton jetty 30 min;",
        "each followed by a 1-min pause (below the 180 s dwell window -> no extra stop).",
...
def city_walk_flapping():
    ...
    while clock < end_time:
        if clock >= next_burst:
            ...
            # traffic-light pause 45-90 s, below the 180 s dwell window -> no stop
            clock = t.dwell(cur, rng.uniform(0.75, 1.5), clock, sample_s=10, jitter_m=8)
            next_pause = clock + timedelta(minutes=rng.uniform(3, 5))
        else:
            heading += rng.uniform(-0.6, 0.6)
            secs = 60
            target = offset(cur[0], cur[1],
                            4.0 / 3.6 * secs * math.cos(heading),
                            4.0 / 3.6 * secs * math.sin(heading))
            clock = t.leg(cur, target, 4.0, 5, clock)
            cur = target

    t.write("city_walk_flapping.gpx", [
        "SYNTHETIC fixture (spec §7): 50-min urban walk torture test, Da'an Taipei.",
        "Generated by generate_fixtures.py, seed=3/30. Random-walk headings, 5 s sampling,",
        "heavy urban GPS noise sigma 12 m.",
        "Base walking 4 km/h with mode-flapping traps:",
        "- every ~5-7 min a 30-45 s burst at 18-22 km/h (< mode_confirm_s=60 -> must not",
        "  confirm a new segment)",
        "- every ~3-5 min a 45-90 s traffic-light pause (< dwell window 180 s -> no stop)",
