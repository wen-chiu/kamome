...
///
/// **The departure's photographs stay.** They are the trip's beginning as the
/// traveller lived it, so the last stop before the crossing — in practice the
/// departure airport, named by `StopNamer` like any other stop — survives the
/// trim with its deck. Everything before it does not.
///
...
public enum RecapTypeTwoFilm {
    ...
    public static func trimmedToTheDestination(_ trip: RecapTrip) -> RecapTrip {
        ...
        return RecapTrip(
            ...
            stops: keepFrom.map { Array(trip.stops[$0...]) } ?? trip.stops,
            title: trip.title,
