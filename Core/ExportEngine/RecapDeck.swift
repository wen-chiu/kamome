...
public struct RecapDeck {
    ...
    /// **The floor no photograph may be shown for less than** (Chiu 2026-08-03).
    ///
    /// `photoHoldS` is what a photo *asks* for; the duration plan then scales
    /// every stop down to fit the film's ceiling, and on a photo-dense trip that
    /// scaling drove the per-photo slot under a second — a flicker rather than a
    /// picture. A stop that cannot afford all its photos at this floor shows
    /// fewer of them; it never shows them faster.
    public var photoMinHoldS: Double = 1.0
    ...
    public init(
        ...
        labelHandoffS: Double = 0.18, photoMinHoldS: Double = 1.0
    ) {
        self.photoHoldS = photoHoldS
        self.labelHandoffS = labelHandoffS
        self.photoMinHoldS = photoMinHoldS
        self.zoomS = zoomS
