...
moved, and it finishes a sequence: **the Worker is hardened** — a 60/min per-IP
burst limit beside the day's ceiling, and **S4 closed**, its no-log property now
a gate that `npm run deploy` runs. **Every precondition to the config flip is met
and the flip itself is Chiu's.** Reflected below, not only in this line.
...
that is deployed, capped, burst-limited and gated no-log (S4/S5 closed), and the
flip itself is two values and **Chiu's decision** — and **D1–D5** (one device
