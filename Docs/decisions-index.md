...
| 72 | `2026-09-12` | The routing key has no build path, and the artifact gate reads it from the environment only | completes the **2026-09-08** flip: removes the `Info.plist` field, the xcconfig include and the template; `check-archive.sh` reads the key from the environment only; three `RoutingKeyTests` restated, none deleted; the desk harness's default endpoint is **not** decided |
