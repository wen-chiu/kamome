...
| 98 | `2026-09-24 (d)` | A tester can hand back the log; device backup is the user's own; the export has a time and memory budget | **Chiu 2026-09-24**, both recommendations. About → "Export diagnostics" in Release (this launch's Kamome log lines, shared by hand, no coordinates); `kamome.sqlite` stays in iCloud device backup (§0 governs what Kamome sends). Also: `Stored` logs repository failures; export failures logged, screen shows domain · code; `export.pipeline` (prefetch 8, concurrency 4, `snapshot_timeout_s` 60, INFERRED) |
| 99 | `2026-09-24 (e)` | An area is framed no deeper than the place it sits in; the pill names the town | **Chiu 2026-09-24.** Context floor per area = parent place (single-linkage ladder over stops, level break `context_level_break_ratio` 3) ÷ `context_depth` 5, within `camera_span_m`…`context_span_max_m` 10 km; all INFERRED. One-area films unchanged. HUD pill: stop when parked, `locality` of the last stop reached on the road, nothing in a crossing; schema v9 `stop.locality`, back-filled on open. Place-name icebox stays frozen. Uncompiled when written. Amends 2026-09-24 §2.1 |
| 100 | `2026-09-24 (f)` | A leg too fast to have been driven is a crossing (DRAFT) | **Chiu 2026-09-24**, reopening `isCrossing` by name. Imported legs with straight-line pace ≥ `crossing_pace_min_kmh` 150 over ≥ 100 km → `beyond_driving`, judged on the phone, never sent to routing; crossing = `no_road` or `beyond_driving`. Elapsed padded by \|Δlon\|/15 h + 2 h margin (+1 day over the antimeridian) so clock/zone errors never fake a flight. Numbers INFERRED |
| 101 | `2026-09-25` | "Day N" is a calendar date; coming home ends a journey | **Chiu 2026-09-25**, from the Japan import. `TripDay`: Day N is the Nth calendar date in the phone's zone, used by S3 chips ("Day 5 · 8/26"), the diary, the Discovery card and the film's HUD and DAYS; it replaces 24 h blocks from the first photo. "Along the route" follows the day chip. R1: a photo at home ends a Discovery journey (second journey on one UTC day keyed `-2`); R2 declined; stored trips do not re-split. Manual split on hold. Country rule built: back in the home country after abroad ends a journey if the step ≥ `homecoming_min_jump_m` 100 km; a photo off any outline (coast buffer `country_coast_buffer_m` 3 km) is no evidence. Natural Earth v5.1.2 offline, 798 KB, keyed ADM0_A3; **Taiwan and China are two countries** — Matsu, Lieyu, Wuqiu, Xiaoliuqiu added to TWN by name |
| 102 | `2026-09-25 (b)` | The person picks a stop's photographs, 1 to 5, and says which stops the film shows | **Chiu 2026-09-25**, reopening PR #91's star / leave-out picker. Tap a numbered photo to take it out, any other to put it in, 1–5; a picked deck is exactly the picks. Photos favourites stay the app's first pick; no in-app star. Stops: ranked ∪ put-in/picked − taken-out; the film grows, nothing replaces a removed stop. Schema v10 `photo_ref.film_pick`, `stop.film_choice`. Smarter auto-pick (Vision) is the next PR |
| 103 | `2026-09-25 (c)` | The app looks at the photographs before it picks them, on the phone, and never makes the film wait | **Engineering, inside (b)'s approved "Not done".** Background Vision per asset (schema v11 `photo_analysis`): screenshot subtype, feature print rev 2, iOS 18 aesthetics (`isUtility`, score). A trip switches once, when every stop photograph has a row; until then the time-based pick, unchanged. Pick: utility never, bursts one moment (`duplicate_distance` 0.3, anchored), best score per slot; no signal = old pick exactly. Utility photos stop counting towards stop rank; quality does not rank stops. iCloud never downloaded in background. Device timings and score fit UNKNOWN — DEBUG probe on Trip Detail |
