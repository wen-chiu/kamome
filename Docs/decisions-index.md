...
| 74 | `2026-09-12 (c)` | The app reproduces the licences of the code it contains, and a pin cannot land without one | GRDB (MIT) and MapLibre (BSD-2-Clause) texts verbatim in About; `check-attribution.sh` keyed on `Package.resolved` and the pinned version; wording and placement are drafts under the S2/S3 rule |
