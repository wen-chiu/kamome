...
| 73 | `2026-09-12 (b)` | The local-network permission belongs to Debug builds, and the shipped plist carries neither key | added by a Debug-only build phase, never removed in Release; localized purpose strings deleted; `check-archive.sh` step 5 checks the artifact; no live LAN workflow found — a designed Debug capability is preserved, not a current one served |
