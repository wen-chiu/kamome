...
| 95 | `2026-09-24` | A stop's photos are picked at their final size; a marked stop always stays; hero photos reopen | **Chiu 2026-09-24.** Decks were an 8-photo spread cut to its first N (first ~30% of a visit only); now `PhotoDeckSelector.pick` after allocation. Marked stops always presented; highlights lift a deck to `import.deck_highlight_max_photos` 5, film priced per deck. `deck_min/max_photos` deleted. No write-back to Photos. Picker: star / leave out (schema v8 `is_excluded`), in the Stop Editor and the export sheet, numbered from the export's own plan. Hero-photo Vision reopened, not started |
