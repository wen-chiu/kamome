...
| 103 | `2026-09-25 (c)` | "No drive path" is asked again on foot: land a car cannot reach is not a crossing | **Chiu 2026-09-25.** Drive `No path` (or unrecognised 400) → ask `mode=walk` with the same waypoints. Walk route without ferry → `off_road_network` (not a crossing); ferry, any 400 or unreadable → `no_road`; walk unanswered → throws, leg stays NULL. Schema v11 clears `no_road` again. From the Iceland Skógar → Seljavallalaug plane. Amends 2026-09-23 (c) |
