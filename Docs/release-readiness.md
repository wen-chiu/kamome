...
| **the routing Worker's deploy config is no-log** — `[observability] enabled = false`, `preview_urls = false`, and no `console.*` in its source. ⚠️ **Half of S4, not all of it**: account-level Logpush, Cloudflare's own retention and `wrangler tail` live in an account this repository cannot see | `check-worker-privacy.sh` **(new 2026-09-05)** |
...
1. **A licence, privacy or §0 obligation that needs a product decision.** Three
   ...
   **S2's placement** and **S3's wording** — both now ship as a working draft, so
   ...
   Chiu 2026-09-04, told once on first run and remembered (ADR 2026-09-05). What
