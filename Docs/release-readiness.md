...
| no routing key is committed, by either path — **and no build input maps one into the app** (ADR 2026-09-12) | `check-secrets.sh` |
| the suite has not silently shrunk (baseline 426) | `check-test-count.sh` |
...
| what is guaranteed | gate |
|---|---|
| the routing key is not in the built artifact — exact match across every file, binaries included; `KamomeRoutingAPIKey` absent or empty; the **shipped** `TrackingConfig.json` is distributable; no 32-hex string in any bundled text resource; **no bundled `Info.plist` carries the local-network permission** (ADR 2026-09-12 (b)) | `release/check-archive.sh` |
...
⚠️ **`check-archive.sh` refuses to run without the real key** — read from
`KAMOME_ROUTING_API_KEY` only, never from a file (ADR 2026-09-12) — rather than falling
...
| ~~**S5**~~ | ✅ **CLOSED 2026-09-04 — the ceiling exists in production.** ~~No per-day budget counter exists in the Worker.~~ `Deploy/worker/src/index.js` refuses above `DAILY_REQUEST_CEILING` (2000/day, Chiu's number) with 429 + `Retry-After` to UTC midnight, and **fails closed at 503** when it cannot count. Deployed from merged `main` (`07fdd14`), **Version ID `5b33922c-0f4e-4d2a-986d-59e8eb7332b4`**. | **VERIFIED from Geoapify's own pages, 2026-08-29:** limits are **soft on every tier**, no customer-settable cap, escalation ends in **account blocking** — which is why the ceiling had to exist here or nowhere. **VERIFIED 2026-09-04 in production:** the after-probe's `/v1/routing` returns **200**, and since *every* KV fault fails closed at 503, that 200 is itself the proof the binding is attached, the ceiling parsed, and the counter read and wrote. `routing-requests-2026-09-04` went **0 → 2** across two probe requests. Suite 19/19; the gate was shown to fire twice (four tests fail when it is neutered; a `wrangler dev` control at ceiling 1 answers the second request 429). | ✅ Done. ⚠️ **What this does NOT close: `matching.base_url` is still `""`**, so no build calls the Worker yet and **every build still carries the routing key**. That was the config flip, and ✅ **it is made — S6 closed 2026-09-08**, so builds now carry no key and the Worker carries the traffic. ⚠️ **A precondition found after this row closed was met first:** the ceiling could not bound a burst inside its own KV cache window, and a **60/min per-IP burst limit** shipped alongside it (ADR 2026-09-05, deployed as Version `09e248ee`). The pair is what the flip waited on; neither half substitutes for the other. |
| ~~**S6**~~ | ✅ **CLOSED 2026-09-08 — the config flip is made, so the key is not a build setting and `xcodebuild` has nothing to echo.** ~~Build logs echo the key in clear text.~~ `matching.base_url` is the Worker and `api_key_required` is `false`, so a shipping build carries no key at all. | **VERIFIED 2026-09-08, end to end, in a checkout with no `Config/Secrets.xcconfig` at all.** The config half is gated: `RoutingKeyTests.testTheShippedConfigPointsAtTheWorkerAndNeedsNoKey` asserts both values and that a keyless build resolves unchanged — **watched red on each half separately**, and reverting `api_key_required` resolves `baseURL` to `""`, i.e. routing silently off, which is the failure the flag exists to prevent. The network half is the **production KV counter**: three desk renders reconstructed 3, 3 and 2 drive legs and the counter moved **+3, +3, +2 to 12**, from a build carrying no key. Legs report `drive/reconstructed`, never `inferred`. | ✅ Done for every future build. 🔴 **Two things this does NOT do.** (1) **Old IPAs still carry the current key** — see S7; the flip only changes what is built from now on. (2) The artifact check is still owed — see the Tier 1 release gates; only `./check.sh --release <artifact>` proves the built bundle carries no key, and it needs the real key, which is Chiu's. ⚠️ **Corrected 2026-09-12:** "a shipping build carries no key" held only on a machine without `Config/Secrets.xcconfig`. The flip left the build path in place, and on a machine keeping that file the key still reached `Info.plist`, the build log and every request to the Worker — **removed by ADR 2026-09-12**, measured with a planted fake key before and after. |
| **S7** | 🟠 **The routing key has never been rotated, and the flip does not retire it.** Every IPA that has ever reached another person's phone contains it (**VERIFIED 2026-08-20:** two built bundles carried a plaintext 32-hex key in `Kamome.app/Info.plist`, read with stock `PlistBuddy`), and so does every build log that was ever taken. The flip stops *new* builds carrying it; it cannot reach into the ones already out. | `Docs/pre-launch.md` item 7 — *"rotate once at submission, after the artifact check passes, so that whatever any development log ever held is dead"* — which was the **only** place this was written down, and `pre-launch.md` is no longer the gate. Recorded here 2026-09-08 because a gate that omits the obligation is how it gets missed. | **Rotate the Geoapify key once, at submission, after `check-archive.sh` passes**, then `wrangler secret put GEOAPIFY_API_KEY`. 🔴 **Chiu's, and only Chiu's** — `wrangler secret put` is never a session's command, and the key never enters a session's environment. Order matters: rotating *before* the artifact check would leave the check validating a bundle nobody will ship. |
...
1. **A licence, privacy or §0 obligation that needs a product decision.** Three
   ...
   **S2's placement** and **S3's wording** — both now ship as a working draft, so
   what reaches Chiu is a revision, not a blank page; and **S3b**, what to do with
   a payload table that describes a state that never arrived. **The fourth is
   answered:** whether the user is told that importing contacts a third party —
   Chiu 2026-09-04, told once on first run and remembered (ADR 2026-09-05 (b)). What
   is left of it is a **deferred** question, not an open obligation: whether the
   user may *refuse*, which needs a mechanism before it needs a button.
