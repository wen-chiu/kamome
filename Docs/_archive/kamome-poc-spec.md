...
#### 4.4.1 Routing profiles per transport mode (scope, Chiu 2026-08-20)

**Kamome's journeys are not car-only.** The product must eventually reconstruct a
journey **whatever the mode** — driving, walking, cycling, public transit. Car ships
first and is the only mode routed today; the rest are a planned extension, and the
architecture must not assume one profile.

**What is true today, and why it looked like a provider limit.**
`RouteMatchService.shouldReconstruct` refuses `.walk` / `.cycle` / `.transit` /
`.unknown` — PD-8, whose stated reason is that *snapping a stroll to the nearest
street invents a journey*. **That reasoning was written against a car-profile
server.** OSM carries `highway=path` / `footway` / `steps`, and a hosted provider
exposes profiles the self-hosted four-region car graph never had. The restriction is
Kamome's, not the router's.

**Decided (Chiu 2026-08-20): walks are routed on a walking profile and drawn solid.**
A real footpath is neither a fabrication nor a dashed guess. Where even a walking
profile has nothing — a beach, a glacier, a field, indoors — the leg stays inferred
and dashed, which becomes the rare case rather than the normal one.

**Rejected in the same conversation: drawing a walk on a road it did not take.** A
hike routed on the driving profile snaps to the nearest service road, often on the
far side of a ridge; a train leg routed on any road profile draws the **expressway**.
Both are the fabrication PD-1/PD-2 exist to prevent, and the honest alternative
already exists.

**Transit is not a routing problem** and must not be solved with a routing profile.
It is the same shape as a ferry: known endpoints, unknown path — handled by the
crossing beat and its own sprite (`Docs/cross-region-journeys.md`).

**Never assume a recorded walk is dashed.** `RecapComposer.provenance(for:)` keys on
the *source*, not the mode: a `.gpsHifi` / `.gpsPassive` walk is `.recorded` and
draws **solid** on the trace the phone actually saw, which is more accurate than any
snap. Only `.exif` / `.timeline` raw geometry is inferred.
...
Renamed & rescoped 2026-07-20 (decisions.md; was "Recap Visual System"). Ships the loop **pick past photos → reconstruct trip → snap to real roads → souvenir-map recap → MP4 → share.** Work order and full detail in `Docs/_archive/handoff-P3.5.md`; the OSRM matching app side is already landed (do not redo). Sequence:
...
2. **MapLibre souvenir-map substrate** — `MapLibreSnapshotProvider` behind the existing `RecapSnapshotProviding` boundary; real geometry + Kamome hand-written **subtractive** style (no generic nav map, no POI noise); MapLibre types confined to that one file; no abstraction layer without a consumer. Guide: `Docs/_archive/vector-tile-pipeline.md`.
...
| Self-hosted vector tiles add ops/size burden | Medium | PMTiles = single static file per region, no tile server; regional extracts only (TW ≈ 100 MB OSM, matching OSRM's footprint); tile generation is a documented offline step (`Docs/_archive/vector-tile-pipeline.md`), not runtime infrastructure. |
