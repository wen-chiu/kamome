# Archived — the type-2 film's three tasks, measured (2026-09-01/02)

**Nothing here is a work instruction.** All three tasks are ✅ built and judged,
and the decisions are in `Docs/decisions.md` (2026-09-01, 2026-09-03 (b),
2026-09-04 (b)), which win. Archived 2026-09-04 under ADR 2026-09-03's rule —
closed work leaves the live corpus. The **closeout is still live** at
`Docs/handoff-type2-films.md`.

What is worth knowing without reading it: MapKit saturates at **~109° of
longitude** so a long-haul frame often does not exist (Taiwan→Iceland fails at
every padding); the shipped threshold is **70°, interpolated** from a gap between
a good shot at 53.2° and a framed-but-useless one at 83.9°; and
`ishigaki-crossing` is a **type 2, not a type 3**, so the tree has no type-3
fixture.
