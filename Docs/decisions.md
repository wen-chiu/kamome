...
## 2026-09-12 (b) — The map credit goes into the film, and this amends 2026-08-17
...
## 2026-09-23 (e) — The beta timeline goes compact: place and date on one line, visits and time at home, provenance by exception
...
visit count no longer multiplies them; ADR 2026-09-23 (d), landed alongside,
stops new repeats and leaves existing ones for Chiu to delete).
...
## 2026-09-23 (f) — The entry gets a drawer; kilometres are ground-only; the visit is a pill

**Decision (Chiu, 2026-09-23)**, on the (e) render: line 2 carries the route with
...
things proposed. This amends (e) points 1 and 3.
...
2. **The drawer opens in place** (chevron, or the VoiceOver action): a scrolling
   ...
   rests on, answering (e)'s INFERRED note.
3. **Ground kilometres** (`LegLength.groundMeters`): legs whose mode is drive,
   ...
   `implausibleRoute`. A `noRoad` leg (a flight or ferry) is out; so is
   `offRoadNetwork` (ADR (c): a beach, but a window-seat photograph reads the
   same); a leg routing has not answered is out too, since it may be a crossing — an undercount that
...
## 2026-09-25 (c) — The app looks at the photographs before it picks them, on the phone, and never makes the film wait
...
1. **Vision runs on the phone, in the background, per photograph.** After an import and whenever a
   ...
   `VNCalculateImageAestheticsScoresRequest` for `isUtility` and `overallScore`. Schema v11
