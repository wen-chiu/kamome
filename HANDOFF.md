...
### 1. ⚠️ The PO session raced the ledger and lost — correctly

This session drafted an ADR for the appearance decision **while the engineering
session was implementing it.** Both entries would have landed in an append-only
ledger, one day apart, on one decision — and the PO draft, being later-dated, would
have *won* by the file's own rule while being the worse entry.

**The engineering session's entry (`2026-08-27`) is authoritative and the PO draft
was withdrawn**, not merged. Theirs is written from the implementation: it carries
the real values (`#FF8A5B`, the cyan it replaced), the rejected alternatives, the
capture boundary, and the ⚠️ persistence gap the PO draft did not know existed.

**And it corrected a fact the PO session got wrong** — the shipped trail was the
`modernMinimal` preset's cyan `(0.42, 0.87, 0.98)`, not `RecapStyle`'s neutral
default `(0.13, 0.45, 0.95)`, which nothing renders. As that entry notes, this was
**the same trap twice from the same source**: the glow brief made it first.

Two rules that follow, and they cost real time to learn:

- **Read a style value off the preset the app selects (`modernMinimal`), never off
  the struct's defaults.** The defaults are unrendered.
- **The PO session does not write an ADR for a decision an engineering session is
  actively implementing.** The implementer's entry is better grounded. The PO
  session records what the implementer will not see — cross-cutting consequences,
  spent guarantees, and what was deliberately left undecided.

### 2. What this pass does contribute

- **ADR 2026-08-27 (b)** — the subject at 157.5 px, and the one thing nothing else
  records: pinning the mark's `length_fraction` at 1.0 **spends** the relational
  guarantee `cb14ae8` built it for. Next time `subject_length_px` moves, the mark's
  size is a fresh judgement.
- **`Docs/camera-arcs.md` §7** — a dated addendum for the points/pixels seam PR #18
  introduced, now updated for the appearance work that landed on top of it: an arc
  composes **two** projection corrections, `pointSize` throws rather than rounding,
  MapKit was measured returning more pixels than requested, and
  `MapKitSnapshotProvider(appearance:)` has no default so Pass 1 must pass one.
  `Docs/eng-session-camera-arc.md` carries the same facts.
- **Three risks** added to `Docs/current-state.md` that were written nowhere: the
  CI billing block, the unexplained 3× raster, and the absence of any spend ceiling.

### 3. 🔴 CI is blocked account-wide, and a red check now means nothing

From 2026-08-29, GitHub Actions jobs fail in ~3 seconds with **zero steps
executed**: *"the job was not started because recent account payments have failed or
your spending limit needs to be increased."* `main` fails identically, so this is
not a branch problem and **not a signal about any PR.**

Until Chiu clears it in GitHub billing, **local `xcodebuild test` is the only
verification there is.** Say that explicitly rather than quoting a green tick you
did not get. There is an irony worth not missing: the session that established "no
spend ceiling exists and the failure is silent" ended on a second spend ceiling
failing silently.

### 4. Awaiting Chiu — a process rule this session may recommend and not write
